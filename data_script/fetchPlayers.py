...
indices = {
    ...
    "player_id":headers.index('PLAYER_ID'),
    "team_id": headers.index('TEAM_ID'),
    "team_abbreviation": headers.index('TEAM_ABBREVIATION'),
    "pts": headers.index('PTS'),
...
filtered_data = [
    {
        "player_name":player_stats_row[indices["player_name"]],
        "player_id":player_stats_row[indices["player_id"]],
        "team_id": player_stats_row[indices["team_id"]],
        "team_abbreviation":player_stats_row[indices["team_abbreviation"]],
        "pts": player_stats_row[indices["pts"]],
        "w_pct": round(player_stats_row[indices["w_pct"]]*100,1),
        "fg_pct": round(player_stats_row[indices["fg_pct"]]*100,1),
        "reb": player_stats_row[indices["reb"]],
        "ast": player_stats_row[indices["ast"]],
        "tov": player_stats_row[indices["tov"]],
        "stl": player_stats_row[indices["stl"]],
        "blk": player_stats_row[indices["blk"]],
        "plus_minus": player_stats_row[indices["plus_minus"]]
    }
    for player_stats_row in player_stats
]
...
for player_data in filtered_data:
    ...
    operation = UpdateOne(
        {'player_id': player_data['player_id']},  # Match document by unique team_id
        {'$set': player_data},  # Set new data
...
# Get all team IDs from the new data
new_player_ids = [player_data['player_id'] for player_data in filtered_data]
